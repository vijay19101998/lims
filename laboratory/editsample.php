...
<div>
	<form method="POST" action="testgroupedit" id="testgroup">
...
// Fetch material format details
$getmaterialFormat = $app->getmaterialFormat($test_category_id, $standard_test_id, $test_grp_id, $sample_id);

// Ensure $getmaterialFormat is an array before looping
if (is_array($getmaterialFormat)) {
    foreach ($getmaterialFormat as $val1) {
        $array1[] = array('parameters' => $val1['parameters'], 'protocols' => $val1['protocols'], 'specification' => $val1['specification'], 'text' => $val1['text'], 'unit' => $val1['unit']);
    }
}
...
// Ensure $result is an array before looping
if (is_array($result)) {
    foreach ($result as $val1) {
        $array2[] = array('parameters' => $val1['parameters'], 'protocols' => $val1['protocols'], 'specification' => $val1['specification'], 'text' => $val1['text'], 'unit' => $val1['unit']);
    }
}

// Merge the arrays and ensure they are arrays
...
if (!empty($array)) {
    foreach ($array as $row) {
        // Fetch material format value
        $result = $app->getmaterialFormatvalue($_REQUEST['id'], $row['parameters'], $row['protocols']);
        
        // Ensure $result is an array before accessing its elements
        if (is_array($result)) {
            if (isset($row['status'])) {
                switch ($row['status']) {
                    case 1:
            ...
            echo '<tr id="additionalContinerId">
                <input class="col form-control form-control-xs" type="hidden" name="id[]" value="' . $result['id'] . '">
                ...
                <td><input class="col-12 form-control ' . $color . ' form-control-xs result" type="text" id="" name="value[]" value="' . $result['value'] . '" tabindex="' . $i++ . '"></td>
                <td><select class="col-12 form-control ' . $color . ' form-control-xs" name="remark[]" fdprocessedid="effusw">
                <option value="">Select</option>
