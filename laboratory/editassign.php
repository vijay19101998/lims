...
<div>
	<form method="POST" action="testgroupeditassign" id="testgroup">
							...
							<div class="mb-3">
								...
								<select required="required" class="form-control js-example-tags" id="material_id" name="material_id">
									...
									foreach ($get_material_group as $row) {
										echo '
                                    <option value="' . $row["name"] . '" ' . (($row["id"] == $material_group) ? 'selected' : '') . '>' . $row["name"] . '</option>';
									}
									?>
								</select>
							</div>
		...
		<div class="table-responsive-md">
			<table class="table table-success" id="st" style="width:100%">
				<thead>
					<tr>
						<th style="color:black;" width="5%">Status</th>
				...
				<tbody id="additionalContiner" class="add-multiform">
					<?php
					echo '<tr id="additionalContinerId' . $snoid . '" class="table-row">
                                    ...
                                    <td><input class="col form-control form-control-xs" type="text" name=""></td>
				...
				<tbody class="additionalContinerClass">
					<!--  -->
					<?php
$getmaterialFormat = $app->getmaterialFormat($test_category_id, $standard_test_id, $test_grp_id, $sample_id);
$array1 = [];
foreach ($getmaterialFormat as $val1) {
    $array1[] = array(
        'parameters' => $val1['parameters'],
        'protocols' => $val1['protocols'],
...
$array = array_unique(array_merge($array1, $array2), SORT_REGULAR);
if (empty($array)) {
    $array = $array2;
}
...
if ($array) {
    foreach ($array as $row) {
        ...
        if (is_array($result) && isset($result['id'])) {
            if ($result['id'] != '') {
                $style = 'style="Color:black;background-color:#15ff00;"';
                $istruevalue = '1';
            }
        }

        echo '<tr id="additionalContinerId">
        <input class="col form-control form-control-xs" type="hidden" name="id[]" value="' . (is_array($result) ? $result['id'] : '') . '">
        <td class="checkbox-cell"><input type="checkbox" value="' . (is_array($result) ? $result['id'] : '') . '" ' . ((is_array($result) && $result['id'] != '') ? 'checked' : '') . ' name="" class="checkbox"></td>
        <td class="input-cell" style="display:none"><input type="text" class="input" value="' . $istruevalue . '" name="istrue[]"></td>
        ...
        <td><input class="col form-control ' . $color . ' form-control-xs" type="text" name="protocols[]" value="' . $row['protocols'] . '" readonly fdprocessedid="3suo2o"></td>
        ...
        <td><input class="col-12 form-control ' . $color . ' form-control-xs result" type="text" id="" name="value[]" value="' . (is_array($result) ? $result['value'] : '') . '" tabindex="' . $i++ . '"></td>
        <td><select class="col-12 form-control ' . $color . ' form-control-xs" name="remark[]" fdprocessedid="effusw">
            <option value="">Select</option>
            <option value="pass" ' . (('pass' == (is_array($result) ? $result['remark'] : '')) ? 'selected' : '') . '>PASS</option>
            <option value="fail" ' . (('fail' == (is_array($result) ? $result['remark'] : '')) ? 'selected' : '') . '>FAIL</option>
        </select></td>
