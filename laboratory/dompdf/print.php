...
$dompdf = new Dompdf();
$sampleData = $app->getReportDetails($id);
$sampleName = strlen($sampleData['sample_name']);
$issuedTo = strlen($sampleData['issued_to']);

// $pg = ($sampleName+$issuedTo)*8;
// print_r($pg);
// exit;
...
<head>
    ...
    <style>
        ' . file_get_contents('https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css') . '
        html {
            font-size: 10px; /* You can adjust this value as needed */
        }

        body, p, h1, h2, h3, h4, h5, h6 {
            font-size: 1rem; /* 1rem is equal to the font size set on the html element */
        }
      //   @page {
      //     margin:150px 30px;
      // }
...
<body>
    ...
    <header class="container-fluid divhead">
    <table class="fixed-div" width="100%">
    <tbody>
        <tr>
            <td>Name of the Sample</td>
            <td>'.$sampleData['sample_name'].'</td>
            <td>Report No</td>
            <td>'.$sampleData['report_number'].'</td>
        </tr>
        <tr>
            <td>Sample Code</td>
            <td>'.$sampleData['sample_code'].'</td>
            <td>Report Date</td>
            <td>'.$sampleData['report_date'].'</td>
        </tr>
        <tr>
            <td>Issued To</td>
            <td>'.$sampleData['issued_to'].'</td>
            <td>Sample Received On</td>
            <td>'.$sampleData['sampling_receive_date'].'</td>
        </tr>
        <tr>
            <td>Test Starting Date</td>
            <td>'.$sampleData['test_start_date'].'</td>
            <td>Test Ending Date</td>
            <td>'.$sampleData['test_end_date'].'</td>
        </tr>
        <tr>
            <td>Sample Quantity</td>
            <td>'.$sampleData['sample_quantity'].'</td>
            <td>Sampling Method</td>
            <td>'.$sampleData['sample_method_id'].'</td>
        </tr>
        <tr>
            <td>Discipline</td>
            <td>'.$sampleData['test_grp_id'].'</td>
            <td>Group</td>
            <td>'.$sampleData['material_id'].'</td>
        </tr>
    </tbody>
</table>';
if (!empty($sampleData['otherInfo'])) {
  $html .= '
  <table border="1">
    <thead>
        <tr>
        ...
        <tr>
            <th colspan="2">Details/Information given by the customer</th>
        </tr>
    </thead>
    <tbody>';
    $otherSno = 0;
    foreach ($sampleData['otherInfo'] as $otherInfoKey => $otherInfoValue) {
      $otherSno++;
      $html .= '<tr>
      <td>'.$otherSno.')'.$otherInfoKey.'</td>
      <td>'.$otherInfoValue.'</td>
  </tr>';
    }
  ...
  foreach ($grpnamedata as $grpkey => $row) {
    ...
    $html .= '
                <tr>
                    ...
                    <td style="' . $hide . '" class="ff" >' . $specification . '</td>
...
$html .= '                   
      ...
      $html .= '<footer>
      <div><b>'.$sampleData['approve_date'].'</b></div>
              <div>('.$sampleData['approve_date'].')</div>
      </footer>';
