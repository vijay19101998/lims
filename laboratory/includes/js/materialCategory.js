$(function() {
  ...
  $('#addmatcat').click(function(){
		...
		$('#matcatForm')[0].reset();
    $('.modal-title').html("<i class='fa fa-plus'></i> Add Material Category");
		$('#action').val('Add');
  ...
  //
  var demoDatatable = $('#matcatList').DataTable({
    'processing': true,
    'serverSide': true,
    'serverMethod': 'post',
    'ajax':{
  ...
  $.validator.setDefaults({
    submitHandler: function(form) {
      ...
      var formData = new FormData(form);
        ...
        $('#matcatList').DataTable().ajax.reload();
          }
  ...
  $(document).on('click', '.update', function(){
    //alert("updat js");
    var id = $(this).attr("id");
    var form_action = 'tbl_material_category';
