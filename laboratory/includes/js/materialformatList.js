$(function () {
  ...
  $('#addmaster').click(function () {
    ...
    $('#mastersubmitform')[0].reset();
    $('.modal-title').html("<i class='fa fa-plus'></i> Add Sample Master Test");
    $('#action').val('Add');
  ...
  //
  var demoDatatable = $('#masterFormatList_').DataTable({
    'processing': true,
    'serverSide': true,
    'serverMethod': 'post',
    'ajax': {
  ...
  $.validator.setDefaults({
    submitHandler: function (form) {
      ...
      $.ajax({
        ...
        $('#masterFormatList').DataTable().ajax.reload();
        }
  ...
  $(document).on('click', '.update', function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        // console.log(data);
        $('#masterModal').modal('show');
        $('#mateial_category_id').val(data.mateial_category_id);
        $('#name').val(data.name);
    ...
    const swalWithBootstrapButtons = Swal.mixin({
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.value) {
          ...
          //
          var id = $(this).attr("id");
          var form_action = 'delete_material_format';
          var action = 'delete_material_format';
          $.ajax({
            url: 'ajax-query/masterSubmit.php',
            method: "POST",
            data: { id: id, action: action, form_action: form_action },
