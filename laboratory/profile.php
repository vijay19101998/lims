<?php
include_once("includes/includes.php");
Session::checkSession();
$menu_active = "note-creation";
include_once(filePath . "/head.php");
include_once(filePath . "/main-navbar.php");
$getProfile = $app->getProfile();
// print_r($getProfile);
// exit;
?>
<style>
    .user {
        display: inline-block;
        width: 150px;
        height: 150px;
        border-radius: 50%;
        object-fit: cover;
    }
</style>
<nav class="page-breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="#">Home</a></li>
        <li class="breadcrumb-item">Settings</li>
        <li class="breadcrumb-item active" aria-current="page">Profile</li>
    </ol>
</nav>
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-baseline">
                    <h6 class="card-title mb-0">Profile</h6>
                    <!-- <button class="btn btn-sm btn-primary btn-icon-text" id="adduser">
                        <i data-feather="edit" class="btn-icon-prepend"></i> Add
                    </button> -->
                </div>
            </div>
            <div class="card-body">
                <div class="col-md-12">
                    <div class="row">
                        <div class="col-md-3">
                            <img src="data:image/jpeg;base64,"
                                class="user">
                        </div>
                        <div class="col-md-3">
                            <div class="card-body">
                                <h5 class="card-title">Name:</h5>
                                <p class="text-muted mb-2"><?= $getProfile['emp_name'] ?></p>
                                <h5 class="card-title">Email:</h5>
                                <p class="text-muted mb-2"><?= $getProfile['email'] ?></p>
                                <h5 class="card-title">Employee Code:</h5>
                                <p class="text-muted"><?= $getProfile['emp_no'] ?></p>

                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="card-body">
                                <h5 class="card-title">Mobile</h5>
                                <p class="text-muted mb-2"><?= $getProfile['mobile_no'] ?></p>
                                <h5 class="card-title">Gender:</h5>
                                <p class="text-muted mb-2"><?= $getProfile['gender'] ?></p>
                                <h5 class="card-title">Date of Join:</h5>
                                <p class="text-muted"><?= $getProfile['created_at'] ?></p>

                            </div>
                        </div>
                    </div>
                </div>
                <hr>
                <!-- <h5 class="card-title">Re-sample Pending List</h5> -->
                <nav>
                    <div class="nav nav-tabs mt-3" id="nav-tab" role="tablist">
                        <button class="nav-link active" id="nav-home-tab" data-bs-toggle="tab"
                            data-bs-target="#nav-home" type="button" role="tab" aria-controls="nav-home"
                            aria-selected="true">Edit User</button>
                        <button class="nav-link" id="nav-profile-tab" data-bs-toggle="tab" data-bs-target="#nav-profile"
                            type="button" role="tab" aria-controls="nav-profile" aria-selected="false">Password
                            Change</button>
                    </div>
                </nav>
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
                        <div class="card">
                        <form method="post" action="profileSubmit" id="profileForm" >
                            <div class="row mt-2 mx-2">
                                <div class="col-sm-3">
                                    <div class="mb-3">
                                        <label class="form-label">User Name</label>
                                        <input type="text" class="form-control" value="<?= $getProfile['emp_name'] ?>" name="username">
                                    </div>
                                </div><!-- Col -->
                                <div class="col-sm-3">
                                    <div class="mb-3">
                                        <label class="form-label">Email</label>
                                        <input type="text" class="form-control" autocomplete="off" value="<?= $getProfile['email'] ?>" name="email">
                                    </div>
                                </div>
                                <div class="col-sm-3">
                                    <div class="mb-3">
                                        <label class="form-label">Mobile</label>
                                        <input type="text" class="form-control" autocomplete="off" value="<?= $getProfile['mobile_no'] ?>" name="mobile_no">
                                    </div>
                                </div>
  
                                <!-- Col -->
                            </div>
                            <div class="col-sm-2 mb-4 mr-2" style="float:right">
                                    <div class="mt-4">
                                        <input type="submit" class="btn btn-primary submit" value="Save">
                                    </div>
                                </div>
                            </form>
                        </div>
                    </div>
                    <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
                        <div class="card">
                        <form method="post" action="profileSubmit" id="profileFormpass" >
                            <div class="row mt-2 mx-2">
   
                                <!-- Col -->
                                <div class="col-sm-3">
                                    <div class="mb-3">
                                        <label class="form-label">Old Password</label>
                                        <input type="text" class="form-control" name="oldPassword" value="">
                                    </div>
                                </div>
                                <div class="col-sm-3">
                                    <div class="mb-3">
                                        <label class="form-label">New Password</label>
                                        <input type="password" class="form-control" name="newPassword" value="">
                                    </div>
                                </div>
                                <!-- Col -->
                                <input type="hidden" name="form_action" value="updatePassword">
                                <div class="col-sm-2 mb-1 mr-2" style="float:right">
                                    <div class="mt-4">
                                        <input type="submit" class="btn btn-primary btn-sm submit" value="Save">
                                    </div>
                                </div>
                            </div>
                            <!-- <div class="row"> -->

                        </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    </div>
    <?php include_once(filePath . "/footer.php"); ?>
    <?php include_once(filePath . "/js.php"); ?>
    <script src="<?php echo BASE_URL; ?>includes/js/user.js"></script>
    <script>

    </script>
    </body>

    </html>