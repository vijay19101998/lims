...
<div>
	<form method="POST" action="testgroup<?php echo isset($_REQUEST['edit']) ? $_REQUEST['edit'] : ''; ?>" id="testgroup">
		<!-- <form method="POST" action="testgroup<?= $_REQUEST['edit'] ?>" id="testgroup"> -->
						...
						<div class="row">
							...
							<div class="col-sm-4">
								<div class="mb-3">
									...
									<input required="required" type="text" class="form-control datepicker-basic-time" name="sampling_date" id="sampling_date" placeholder="" value="<?= $sampling_date; ?>">
								</div>
							</div>
							<div class="col-sm-4">
								<div class="mb-3">
    ...
    <span class="input-group-text" style="padding: 18px;">Notes</span>
		...
		<button type="submit" class="btn btn-primary float-right mt-4" style="float:right" id="action-btn">
			<?= isset($_REQUEST['edit']) ? ucfirst($_REQUEST['edit']) . '-' : '' ?>Submit
		</button>
	</form>
	<?php if (empty($_REQUEST['view'])) : ?>
		<a href="editsample.php?id=<?= $id ?>" class="btn btn-primary float-right">Edit</a>
	<?php endif; ?>
</div>
...
<script>
	...
	$(document).ready(function() {
		$('#test_category_name').on('change', function() {
			...
			// alert(test_category_id);
			if (test_category_id == 'EXTERNAL SAMPLE') {
				$('#sampling_date').removeAttr('required');
			} else {
				$('#sampling_date').attr('required', true);
			}
		});
	});
</script>
